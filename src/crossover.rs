...
use crate::{EA, ModelItem};

pub fn neighbourhood_crossover(ea: &mut EA) {
    let mut rng = rand::thread_rng();
    let split_index = rng.gen_range(0..ea.objectives.len()) as usize;

    sort_pool_by_objective(ea);
    neighbourhood_shuffle(ea, &mut rng);

    for [p1, p2] in ea.mating_pool.iter_mut().array_chunks() {
        perform_crossover(p1, p2, split_index);
    }
}

fn sort_pool_by_objective(ea: &mut EA) {
    let i = ea.next_objective_sort_index();
    ea
        .mating_pool
        .sort_by(|a, b| a.values[i].partial_cmp(&b.values[i]).unwrap());
}

fn neighbourhood_shuffle(ea: &'_ mut EA, rng: &mut ThreadRng) {
    let ns = ea.neighbourhood_size;
    let pool = ea.mating_pool.as_mut_slice();
